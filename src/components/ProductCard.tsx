...
import { getLocalizedInfoRows } from "@/i18n";
...
export default function ProductCard({
  ...
  const infoRows = getLocalizedInfoRows(product, locale);
  const brandRow = infoRows.find((row) => row.key === "brand")!;
  ...
  return (
    <div className="group relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:border-sky-300 hover:shadow-[0_28px_56px_-24px_rgba(15,23,42,0.2)]">
      ...
      <AddToCartButton productId={product.id} model={product.model} variant="overlay" />
          ...
          <div>
            <span
              className={`text-sm font-bold uppercase tracking-wide sm:text-base ${
                brandRow.isPending ? "italic text-slate-400" : "text-sky-600"
              }`}
            >
              {brandRow.value}
            </span>
            ...
            <p className="mt-2 font-heading text-xl font-bold text-slate-900 sm:text-2xl">
          ...
          <dl className="grid grid-cols-2 gap-x-6 gap-y-2.5 border-t border-slate-100 pt-4 text-sm sm:gap-y-3 sm:pt-5">
            {infoRows.map((row) => (
              <div key={row.key} className="flex flex-col gap-0.5">
                ...
                <dd className={row.isPending ? "italic text-slate-400" : "font-medium text-slate-900"}>
                  {row.value}
                </dd>
              </div>
            ))}
